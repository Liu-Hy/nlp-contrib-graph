            ...
            if line:
                if is_heading(line) and case_check(line, flg):
                    ...
                    if is_main_heading(line):
                        ...
                        sent.append(
                            [i, line, '', '', task, index, None, None, None, '', '', '', '', '', '', '', '', '', None, 1, 0, None])
                    else:
                        ...
                        if is_main_heading(main_h, judge_mask=True):
                            sent.append([i, line, main_h, '', task,
                                         index, None, None, None, '', '', '', '', '', '', '', '', '', None, 0, 0, None])
                        else:
                            sent.append([i, line, main_h, '', task,
                                         index, None, None, None, '', '', '', '', '', '', '', '', '', None, 1, 0, None])
                else:
                    # For plain text line, store both the heading and the main heading.
                    if is_main_heading(main_h, judge_mask=True):
                        sent.append([i, line, main_h, heading,
                                     task, index, None, None, None, '', '', '', '', '', '', '', '', '', None, 0, 0, None])
                    else:
                        sent.append([i, line, main_h, heading,
                                     task, index, None, None, None, '', '', '', '', '', '', '', '', '', None, 1, 0, None])
            else:
                break
                    ...
                    if not evidence:
                        ...
                        if max(lens) == 0:
                            ...
                            for t in range(len(aux[idx][3])):
                                ...
                                found[w] = 1
                            if max(lens) == 3:
                                sent[aux[idx][0]][9] += line + '\n'
                            if max(lens) == 2:
                                if found == [1,0,1]:
                                    sent[aux[idx][0]][10] += line + '\n'
                                elif found == [0, 1, 1]:
                                    uni_name = unit[:-4]
                                    if triple[0] == (uni_name[0].upper()+uni_name[1:]).replace('-', ' '):
                                        sent[aux[idx][0]][11] += line + '\n'
                                    else: #SPEC A
                                        sent[aux[idx][0]][14] += line + '\n'
                                else: # SPEC B
                                    sent[aux[idx][0]][15] += line + '\n'
                            if max(lens) == 1:
                                if found == [0, 0, 1]:
                                    uni_name = unit[:-4]
                                    if triple[0] == (uni_name[0].upper()+uni_name[1:]).replace('-', ' '):
                                        sent[aux[idx][0]][12] += line + '\n'
                                    elif triple[0] == 'Contribution':
                                        sent[aux[idx][0]][13] += line + '\n'
                                    else: # SPEC C
                                        sent[aux[idx][0]][16] += line + '\n'
                                else: # SPEC D
                                    sent[aux[idx][0]][17] += line + '\n'
...
df.columns = ['idx', 'text', 'main_heading', 'heading',
              'topic', 'paper_idx', 'BIO', 'BIO_1', 'BIO_2', 'triple_A', 'triple_B', 'triple_C', 'triple_D', 'triple_E', 'SPEC_1', 'SPEC_2', 'SPEC_3', 'SPEC_4', 'phrases', 'mask', 'bi_labels', 'labels']
df1 = df[['labels', 'text', 'phrases', 'triple_A',
          'triple_B', 'triple_C', 'triple_D', 'triple_E', 'SPEC_1', 'SPEC_2', 'SPEC_3', 'SPEC_4']].rename(columns={'labels': 'info_unit'})
# df1.to_csv('./interim/all_sent.csv', index=False)
pos = df1[df['bi_labels'] == 1]
pos.to_csv('./interim/triples.csv', index=False)

with open('./interim/cross_sentence_triples.txt', 'w') as f:
    for item in reports:
        f.write('%s\n' % item)
with open('./interim/not_found_triples.txt', 'w') as f:
    for tri in special_trip:
        f.write('%s\n' % tri)
print('\n\"triples.csv\", \"cross_sentence_triples.txt\" and \"not_found_triples.txt\" have been saved to ./interim')
