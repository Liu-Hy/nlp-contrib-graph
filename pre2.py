            ...
            if line:
                ...
                if is_heading(line) and case_check(line, flg):
                    ...
                    if is_main_heading(line):
                        ...
                        sent.append(
                            [i, line, '', '', task, index, None, None, None, ofs1, 0, i-1, 0, ofs3, 0, 1, 0, []])
                    else:
                        ofs1 += 1
                        ...
                        if is_main_heading(main_h, judge_mask=True):
                            sent.append([i, line, main_h, '', task,
                                         index, None, None, None, ofs1, 0, i-1, 0, ofs3, 0, 0, 0, []])
                        else:
                            sent.append([i, line, main_h, '', task,
                                         index, None, None, None, ofs1, 0, i-1, 0, ofs3, 0, 1, 0, []])
                else:
                    # For plain text line, store both the heading and the main heading.
                    ofs1 += 1
                    if is_main_heading(main_h, judge_mask=True):
                        sent.append([i, line, main_h, heading, 
                                     task, index, None, None, None, ofs1, 0, i-1, 0, ofs3, 0, 0, 0, []])
                    else:
                        sent.append([i, line, main_h, heading, 
                                     task, index, None, None, None, ofs1, 0, i-1, 0, ofs3, 0, 1, 0, []])
            else:
                break
    ...
    for unit in os.listdir(j_dir):  # For each json file representing an information unit
        ...
        try:
            ...
            if "TITLE" in lst:  # When the title is a source sentence, sometimes it is abbreviated as 'TITLE'
                sent[1][-1].append(unit[:-5])
            for j in range(len(sent)):
                if sent[j][1] in lst:
                    sent[j][-1].append(unit[:-5])
...
df.to_csv('./interim/all_sent2.csv', index=False)
pos = df[df['bi_labels'] == 1]
pos.to_csv('./interim/pos_sent2.csv', index=False)
print('\n\"all_sent2.csv\" and \"pos_sent2.csv\" have been saved to ./interim')
