...
from simpletransformers.classification.classification_utils1 import (
    InputExample,
...
from simpletransformers.classification.transformer_models.bert_model1 import BertForSequenceClassification
...
from simpletransformers.config.model_args1 import ClassificationArgs1
...
class ClassificationModel1:
    def __init__(
        ...
        **kwargs,
        ...
        elif isinstance(args, ClassificationArgs1):
            self.args = args
    ...
    def train_model(
        ...
        **kwargs,
        ...
        if self.args.downsample > 1.0:
            ...
            train_neg = train_neg.sample(frac=(1.0/self.args.downsample), random_state=1)
            train_df = pd.concat([train_pos, train_neg])
            train_df = train_df.sample(frac=1, random_state=1)
            len_after = len(train_df)
            d_r = 100.0 - round((len_after/len_before), 4) * 100
            print(f'{d_r}% of the samples are removed by dowmsampling')
        ...
        if isinstance(train_df, str) and self.args.lazy_loading:
            ...
            if "text" in train_df.columns and "labels" in train_df.columns:
                if self.args.model_type == "layoutlm":
                    ...
                    train_examples = [
                        InputExample(i, text, title, offset1, pro1, offset2, pro2, offset3, pro3, None, label)
                        for i, (text, title, offset1, pro1, offset2, pro2, offset3, pro3, label) in enumerate(zip(train_df["text"].astype(str), train_df["title"].astype(str), train_df["offset1"], train_df["pro1"],
                            train_df["offset2"],train_df["pro2"], train_df["offset3"],train_df["pro3"], train_df["labels"]))
                    ]
    ...
    def train(
        ...
        **kwargs,
        ...
        if args.optimizer == "AdamW":
            ...
            #optimizer = AdamW([{'params': model.bert.pooler.parameters()}, {'params': model.bert.encoder.layer[11].parameters()}, {'params': model.bert.encoder.layer[10].parameters()}, {'params': model.bert.encoder.layer[9].parameters()},
                               #{'params': model.classifier1.parameters(), 'lr': args.out_learning_rate}, {'params': model.classifier2.parameters(), 'lr': args.out_learning_rate}], lr=args.learning_rate, eps=args.adam_epsilon)
        ...
        def get_noam_schedule_with_warmup(optimizer, num_warmup_steps, last_epoch=-1):
            def lr_lambda(current_step: int):
                ...
                return (0.2 / args.learning_rate) * (768 ** (-0.5)) * min(step ** (-0.5), step * num_warmup_steps ** (-1.5))
        ...
        elif args.scheduler == "noam_schedule_with_warmup":
            scheduler = get_noam_schedule_with_warmup(
                optimizer,
                num_warmup_steps=args.warmup_steps,
            )
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                if self.args.fp16:
                    ...
                    outputs = model(**inputs)
                    #print(model.bert.encoder.layer[0].parameters())
                    # model outputs are always tuple in pytorch-transformers (see doc)
                    loss = outputs[0]
        ...
        if isinstance(eval_df, str) and self.args.lazy_loading:
            ...
            if "text" in eval_df.columns and "labels" in eval_df.columns:
                if self.args.model_type == "layoutlm":
                    ...
                    eval_examples = [
                        InputExample(i, text, title, offset1, pro1, offset2, pro2, offset3, pro3, None, label)
                        for i, (text, title, offset1, pro1, offset2, pro2, offset3, pro3, label) in enumerate(zip(eval_df["text"].astype(str), eval_df["title"].astype(str), eval_df["offset1"], eval_df["pro1"],
                            eval_df["offset2"], eval_df["pro2"], eval_df["offset3"], eval_df["pro3"], eval_df["labels"]))
                    ]
    ...
    def load_and_cache_examples(
        ...
        all_segment_ids1 = torch.tensor([f.segment_ids1 for f in features], dtype=torch.long)
        ...
        all_offset1 = torch.tensor([f.ofs1 for f in features], dtype=torch.float)
        all_pro1 = torch.tensor([f.pro1 for f in features], dtype=torch.float)
        all_offset2 = torch.tensor([f.ofs2 for f in features], dtype=torch.float)
        all_pro2 = torch.tensor([f.pro2 for f in features], dtype=torch.float)
        all_offset3 = torch.tensor([f.ofs3 for f in features], dtype=torch.float)
        all_pro3 = torch.tensor([f.pro3 for f in features], dtype=torch.float)
        #print(all_offsets.shape)

        if args.normalize_ofs:
            def nml(arr):
                means = arr.mean()
                stds = arr.std()
                arr = (arr - means) / stds
                return arr
            all_offset1 = nml(all_offset1)
            all_pro1 = nml(all_pro1)
            all_offset2 = nml(all_offset2)
            all_pro2 = nml(all_pro2)
            all_offset3 = nml(all_offset3)
            all_pro3 = nml(all_pro3)
        
        if self.args.model_type == "layoutlm":
            dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_bboxes)
        else:
            dataset = TensorDataset(all_input_ids1, all_input_mask1, all_segment_ids1, 
                                    all_input_ids2, all_input_mask2, all_segment_ids2, 
                                    all_offset1, all_pro1, all_offset2, all_pro2, all_offset3, all_pro3, all_label_ids)
    ...
    def _get_inputs_dict(self, batch):
        if isinstance(batch[0], dict):
            ...
            inputs = {"input_ids1": batch[0], "attention_mask1": batch[1], "token_type_ids1": batch[2],
                      "input_ids2": batch[3], "attention_mask2": batch[4], "token_type_ids2": batch[5],
                      "ofs1": batch[6], "pro1": batch[7], "ofs2": batch[8], "pro2": batch[9], "ofs3": batch[10], "pro3": batch[11], "labels": batch[12]}
