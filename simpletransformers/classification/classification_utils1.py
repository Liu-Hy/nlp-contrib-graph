...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_t, ofs1, pro1, ofs2, pro2, ofs3, pro3, text_b=None, label=None, x0=None, y0=None, x1=None, y1=None):
        """
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids1, input_mask1, segment_ids1, 
                 input_ids2, input_mask2, segment_ids2, 
                 ofs1, pro1, ofs2, pro2, ofs3, pro3, label_id, bboxes=None):
...
def convert_example_to_feature(
    ...
    if add_prefix_space and not example.text_t.startswith(" "):
        tokens_t = tokenizer.tokenize(" " + example.text_t)
    else:
        tokens_t = tokenizer.tokenize(example.text_t)
    ...
    if len(tokens_t) > max_seq_length - special_tokens_count:
        tokens_t = tokens_t[: (max_seq_length - special_tokens_count)]


    tokens1 = tokens_a + [sep_token]
    segment_ids1 = [sequence_a_segment_id] * len(tokens1)
    tokens2 = tokens_t + [sep_token]
    segment_ids2 = [sequence_a_segment_id] * len(tokens2)

    if cls_token_at_end:
        tokens1 = tokens1 + [cls_token]
        segment_ids1 = segment_ids1 + [cls_token_segment_id]
        tokens2 = tokens2 + [cls_token]
        segment_ids2 = segment_ids2 + [cls_token_segment_id]

    else:
        tokens1 = [cls_token] + tokens1
        segment_ids1 = [cls_token_segment_id] + segment_ids1
    ...
    input_mask1 = [1 if mask_padding_with_zero else 0] * len(input_ids1)
    input_mask2 = [1 if mask_padding_with_zero else 0] * len(input_ids2)

    # Zero-pad up to the sequence length.
    if pad_to_max_length:
        padding_length1 = max_seq_length - len(input_ids1)
        padding_length2 = max_seq_length - len(input_ids2)
        if pad_on_left:
            input_ids1 = ([pad_token] * padding_length1) + input_ids1
            input_ids2 = ([pad_token] * padding_length2) + input_ids2
            input_mask1 = ([0 if mask_padding_with_zero else 1] * padding_length1) + input_mask1
            input_mask2 = ([0 if mask_padding_with_zero else 1] * padding_length2) + input_mask2
            segment_ids1 = ([pad_token_segment_id] * padding_length1) + segment_ids1
            segment_ids2 = ([pad_token_segment_id] * padding_length2) + segment_ids2
        else:
            input_ids1 = input_ids1 + ([pad_token] * padding_length1)
            input_ids2 = input_ids2 + ([pad_token] * padding_length2)
            input_mask1 = input_mask1 + ([0 if mask_padding_with_zero else 1] * padding_length1)
            input_mask2 = input_mask2 + ([0 if mask_padding_with_zero else 1] * padding_length2)
            segment_ids1 = segment_ids1 + ([pad_token_segment_id] * padding_length1)
    ...
    return InputFeatures(
        input_ids1=input_ids1, input_mask1=input_mask1, segment_ids1=segment_ids1, 
        input_ids2=input_ids2, input_mask2=input_mask2, segment_ids2=segment_ids2, 
        ofs1=example.ofs1, pro1=example.pro1, ofs2=example.ofs2, pro2=example.pro2, 
        ofs3=example.ofs3, pro3=example.pro3, label_id=example.label,
    )
