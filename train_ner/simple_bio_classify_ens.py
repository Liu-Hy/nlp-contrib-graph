...
for i in range(8):
    idx = np.random.randint(len(train_df), size=len(train_df))
    print(idx[:30])
    model_args.output_dir='classify_sub'+str(i)
    model_args.best_model_dir = 'classify_sub'+str(i)+'/best_model'
    # 25326
    ...
    t_d=t_d.iloc[idx]
    # Train the model
    model.train_model(t_d, eval_df=eval_df,F1_score=sklearn.metrics.f1_score)
